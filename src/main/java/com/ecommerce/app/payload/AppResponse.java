...
public record AppResponse<T>(
    ...
    public static <T> AppResponse<T> failedAppResponse(Error error, HttpStatus status, String requestId) {
        return new AppResponse<T>(null, false, status.value(), error, new Date(), requestId);
    }
